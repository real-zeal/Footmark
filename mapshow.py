...
df = pd.read_table('data/data.csv')  # 读取目标文件
df['lon'] = 6.0  # 插入lon列
df['lat'] = 7.0  # 插入lat列
...
def getStaticAmap(lonlat_str, str_city_center):  # <---获取静态高德地图--->
    ...
    #高德地图-->静态地图API地址
    url = r'http://restapi.amap.com/v3/staticmap?location=%s&zoom=10&size=1024*768&key=<320f6fdbf528ac0ccd282b666c3cc7fb>'
...
if __name__ == '__main__':  # <---主程序--->
    ...
    while i < 15:  # 循环,高德地图有<50个点限制
          ...
          lonlat_str = formatLocation(
              lonlat_str, getLocation)  # 定义marker点格式--->静态地图用
          # print lonlat_str                        #显示marker点
          # print df[i:i+1]                         #显示相关行数据
          i += 1
    # print df                                       #观察一下数据
    getStaticAmap(lonlat_str, str_city_center)  # 打开静态地图
    df.to_csv('WWT_lonlat.csv', index=False,
              sep=',', encoding='utf-8')  # 存储为csv格式
